...
jest.mock('pages/SubscriptionRenewal/SelectPlan/hooks/usePlanList', () => {
  return {
    __esModule: true,
    default: jest.fn(),
  };
...
const userPlanList: SubscriptionPlan[] = [basicPlan, proPlan];

describe('PlanList', () => {
  it('should render plan list correctly', () => {
    (usePlanList as jest.Mock).mockReturnValue({
      planList: userPlanList,
      userPlan: userPlanList[0],
    });
    render(<PlanList />, {
      wrapper: TestRoot,
    });

    const planItemElements = screen.getAllByTestId('plan-item');

    expect(planItemElements).toHaveLength(userPlanList.length);
  });
});
