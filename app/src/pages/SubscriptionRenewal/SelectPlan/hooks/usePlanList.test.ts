...
jest.mock('../../hooks/useSelectSubscriptionPlan', () => {
  return {
    __esModule: true,
    default: jest.fn(), // Create a mock function for useSelectSubscriptionPlan
  };
});
...
describe('usePlanList', () => {
  it('should return data successfully', async () => {
    ...
    const { result } = renderHook(() => usePlanList(), { wrapper: TestRoot });

    expect(result.current.isLoading).toBe(true);
    expect(result.current.planList).toStrictEqual([]);
    expect(result.current.userPlan).toBe(undefined);
    await waitFor(() => {
      expect(result.current.isLoading).toBe(false);
      // expect(result.current.planList).toStrictEqual(userPlanList);
      // expect(result.current.userPlan).toStrictEqual(basicPlan);
